...
! Constituent, tributary, and widthdrawal totals
  ...
  ALLOCATE (VISCOS(KMX, IMX), PARZ(KMX, IMX), ARHOZ(KMX, IMX), ASCYA(KMX, IMX)) ! CSW 1/4/17
  ALLOCATE (ISFIRST(KMX, IMX), SJDAY(KMX, IMX), EJDAY(KMX, IMX), DLIGHT(KMX, IMX), PARA(KMX, IMX), SPARZ(KMX, IMX)) ! CSW 1/7/17
...
! Derived variables
  ...
  CBODU => CD(:,:,19); PH   => CD(:,:,20); CO2  => CD(:,:,21); HCO3 => CD(:,:,22); CO3   => CD(:,:,23); DCELL => CD(:,:,24)   ! CSW 1/3/17
  CCELL => CD(:,:,25); GCELL => CD(:,:,26)                                                                                    ! CSW/1/3/17 
...
! Kinetics coefficients
  ...
  !++CSW Read Geosmin parameters
  READ (CON,'(//(8X,6F8.0,I8,3F8.0))') (AP(JA),     AN(JA),     AC(JA),     ASI(JA),    ACHLA(JA), APOM(JA),                        &
                                       ANEQN(JA),  ANPR(JA),  AGEOS(JA), CACEL(JA),  JA=1,NAL)
  !--CSW
